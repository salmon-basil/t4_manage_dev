...
const seedTestData = () => {
    ...
    const insertProfile = db.prepare(
        "INSERT OR IGNORE INTO Profile (userId, displayName) VALUES ((SELECT id FROM User WHERE username = ?), ?)",
    );
    const insertRank = db.prepare(
        "INSERT OR IGNORE INTO Rank (userId, weeklyMinutes, rank, league) VALUES ((SELECT id FROM User WHERE username = ?), ?, ?, ?)",
    );

    const insertStudy = db.prepare(
        "INSERT OR IGNORE INTO StudyRecord (userId, studyDate, studyMinutes) VALUES ((SELECT id FROM User WHERE username = ?), ?, ?)",
    );
    ...
    for (const user of users) {
        ...
        insertStudy.run(
            user.username,
            today,
            Math.max(30, Math.floor(user.weeklyMinutes / 7)),
        );
    }
};
